"""
Window-corrected PTA anisotropy experiment using the modular pta_anisotropy package.

This mirrors oowindow.py but relies on the shared modules and uses the
window-corrected frequency integration kernels.
"""
...
def main():
    ...
    # Outputs now live under ./output_dir/windowed
    output_root = constants.get_output_directory(base_path, "output_dir")
    output_dir = constants.get_output_directory(output_root, "windowed")

    print("=" * 70)
    print("FLEXIBLE FREQUENCY-DEPENDENT ANISOTROPY ANALYSIS (WINDOWED)")
    print("=" * 70)

    # Simulation parameters (mirroring oowindow.py)
    N_runs = 500
    NN_pulsars = [300]
    Tspan_yrs = 16.0
    ...
    # Test cases (copied from oowindow.py)
    test_cases = [
        {
    ...
    for scenario in scenarios:
        ...
        for npulsars in NN_pulsars:
            ...
            for nn in tqdm.tqdm(range(N_runs), desc="    Processing"):
                ...
                D_IJ_full = data_model.get_D_IJ_fifj_windowed(
                    Tspan,
                    fi_full,
                    ff,
                    jnp.array(h_tilde),
